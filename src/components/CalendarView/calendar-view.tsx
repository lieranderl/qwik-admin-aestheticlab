...
export const BookingCalendar = component$(({ selectedTechnicians, calendarStore }: CalendarProps) => {
    ...
    useOnDocument('DOMContentLoaded', $(() => {
        if (!calendarInstance.value && calendarRef.value) {
            const calendar = AestheticCalendar(calendarRef.value, EventClick, SelectClick, eventResize, eventDrop, calendarStore)
            calendar.render();
    ...
    useTask$(async ({ track }) => {
        ...
        if (selectedTechnicians.value) {
            const bookings = selectedTechnicians.value.map(async (technician) => {
                return await fetchBookings(technician.id, technician.name, technician.color, services.value.data || [], calendarStore.activeStart.toISOString(), calendarStore.activeEnd.toISOString());
            });

            const BookingResponse = await Promise.all(bookings);
            calendarStore.events = BookingResponse.map((b) => b.data || []).flat();
